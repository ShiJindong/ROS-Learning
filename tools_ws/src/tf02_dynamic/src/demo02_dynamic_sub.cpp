...
int main(int argc, char *argv[])
{
    ...
    ps.header.frame_id = "turtle1";             // 该点为turtle1坐标系下的点
    ps.header.stamp = ros::Time(0.0);           // 注意，不再是ros::Time::now()     
    /* 
    ...
    while(ros::ok())    // 雷达会不间断接受点的消息，这里用while循环进行模拟
    {
        ...
        */
        ...
        try
        {
            ...
            ps_out = buffer.transform(ps, "world");     // 参数1: 被转换的点 参数2: 目标坐标系  返回值: 输出转换后的点
            // 6. 最后输出 
